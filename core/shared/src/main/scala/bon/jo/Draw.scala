...
object Draw {
  ...
  extension [T](e: Positioned[Grid[T]] with Access)
    ...
    def canWriteAndIsIn(xp: Int, yp: Int): Boolean =
      e.canWrite && e.isIn(xp, yp)
  type SheetMV[T, V] = ModelAndView[Positioned[Grid[T]], V]

  object FrameGrid:
    def unapply[T](
        f: FrameGrid[T]
    ): Some[(Int, Int, Seq[Array[GridElement[T]]])] =
      Some(f.xSize, f.ySize, f.frames)
  class FrameGrid[T](override val xSize: Int, override val ySize: Int)
      extends Grid[T](xSize: Int, ySize: Int):
    ...
    override def resetData(dataS: List[GridValueExport[T]]): Unit =
      for (i <- 0 until xSize * ySize) {
        frames(currentFrame)(i) = EmptyGridElement
      }
      dataS.foreach { d =>
        frames(currentFrame)(d.xy) = GridValue(d.v)
      }
    ...
    def addFrame() =
      ...
      frames = frames :+ Array.copyOf(fd, fd.length)

  class MasterGrid[T](override val xSize: Int, override val ySize: Int)
      extends Grid[T](xSize: Int, ySize: Int):
    override def json(): All.ObjectAll[String] =
      var base = super.json()
      base + obj(
        "sheets" := All.ListAll[String](
          sheet.map(v => obj("x" := v.x, "y" := v.y, "m" := v.moveString) + v.v.json())
        )
      )
    var sheet
        : List[Positioned[Grid[T]] with Access with AccessVar with Moving[T]] =
      Nil
    override def apply(xp: Int, yp: Int): GridElement[T] =
      sheet
        .filter(_.canReadAndIsIn(xp, yp))
        .flatMap { p =>
  ...
  extension [T](v: All.ObjectAll[String])
    def toGridValueExport: GridValueExport[T] = GridValueExport(
      (v / "v").value,
      (v / "xy").asValue[Any]._value match
        case e: Int  => e
        case e: Long => e.toInt
    )
  trait NormalAccessGrid[T]:
    ...
    def update(xp: Int, yp: Int, v: GridElement[T]): Unit = data(
      coord(xp, yp)
    ) = v
    inline def update(xp: Int, yp: Int, v: T): Unit = this(xp, yp) = GridValue(
      v
    )

  class Grid[T](val xSize: Int, val ySize: Int)
      extends GridOps[T]
      with NormalAccessGrid[T]
  trait GridOps[T]:
    val xSize: Int
    val ySize: Int
    val data: Array[GridElement[T]] = emptyData()
    def emptyData(): Array[GridElement[T]] = Array(
      (for (i <- 0 until xSize * ySize) yield EmptyGridElement)*
    )
    ...
    def exportFun(): Iterable[GridValueExport[T]] =
      exportFun(data)
    inline def exportFun(
        d: Iterable[GridElement[T]]
    ): Iterable[GridValueExport[T]] =
      d.zipWithIndex
        .filter(_._1 != EmptyGridElement)
        .map((e, i) => GridValueExport(e.asGridValue[T]().v, i))
    def json(): All.ObjectAll[String] =
      obj("xSize" := xSize, "ySize" := ySize) + (
        this match
          case FrameGrid(_, _, frames) =>
            obj(
              "frames" := All.ListAll(
                frames
                  .map(exportFun(_).map(_.toAllObj))
                  .map(e => All.ListAll(e.toList))
                  .toList
              )
            )
          case o =>
            obj("data" := All.ListAll(exportFun().map(_.toAllObj).toList))
      )

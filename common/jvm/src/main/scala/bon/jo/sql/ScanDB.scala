...
object ScanDB:
  given Conversion[ResultSet, (String, model.Table)] =
    r =>
      r.getString("TABLE_SCHEM") -> model.Table(
  ...
  def apply(e: Connection): model.DB =
    ...
    var tables_ = e
      ...
      .to[(String, model.Table)]
      .toSeq
    var columns_ = e
      .getMetaData()
      .getColumns(null, null, null, null)
      .to[(String, String, model.Column)]
      .toSeq
    schemas_.foreach(println)
    tables_.foreach(println)
    columns_.foreach(println)

    val pks = tables_.filter(_._2.tableType == "TABLE").map { t =>
      {
        val fks = e
          .getMetaData()
          .getExportedKeys(null, null, t._2.name)
          .toRawNamedList
          .toSeq
        println("=" * 100)
        fks.foreach(println)
        ...
        t._1 -> t._2.copy(pks =
          e.getMetaData()
            .getPrimaryKeys(null, t._1, t._2.name)
            .toRawNamedList
            .toSeq
            .map(_.find(_._1 == "COLUMN_NAME").get)
            .map(_._2.toString())
        )
      }
    }
    ...
    pks.foreach(println)
    println("=" * 100)
    val (schemas__, tables__, columns__) =
      if schemas_.isEmpty then
        (
          Seq(model.Schema("root", Nil)),
          tables_.map((sc, t) => if sc == null then "root" -> t else sc -> t),
          columns_.map((sc, t, c) =>
            if sc == null then ("root", t, c) else (sc, t, c)
          )
        )
      else (schemas_, tables_, columns_)

    val tablesMap = tables__.groupBy(_._1).mapValues(_.map(_._2)).toMap
    val columnsMap = columns__
      .groupBy(_._1)
      .mapValues(
        _.map((_, t, c) => t -> c)
          .groupBy(_._1)
    ...
    val fullSchema = schemas__.map { s =>
      s.copy(tables = {
        tablesMap(s.name).map { t =>
          t.copy(columns = columnsMap(s.name).getOrElse(t.name, Nil))
        }
      })
    }
  ...
  def test(): Unit =
    val con = DriverManager.getConnection(
      """jdbc:sqlite:C:\Users\Jonathan\Desktop\objectModel\game-engine\sample2.db"""
    )
    println("=" * 100)
    println(ScanDB(con))
    con.close()
